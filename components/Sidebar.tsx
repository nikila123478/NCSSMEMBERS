...
import { LayoutDashboard, Wallet, CheckSquare, Users, PenTool, Image, Newspaper, Bell, MessageSquare, LogOut, Home, Shield, ExternalLink, FileText, Calendar } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, role }) => {
  ...
  // Items visible to ONLY Super Admin (Now including Members/Gallery/News)
  ...
  // Make adminItems effectively empty or removed
  const adminItems = [];
    ...
    <aside className="fixed inset-y-0 left-0 w-72 glass-panel border-r-0 z-30 pt-24 pb-6 hidden md:block m-4 rounded-3xl h-[calc(100vh-2rem)]">
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto py-2 px-4 space-y-1 custom-scrollbar">
          ...
          {/* Admin Items (Hidden if empty) */}
          {['SUPER_ADMIN', 'super_admin', 'MEMBER_ADMIN', 'admin'].includes(role || '') && adminItems.length > 0 && (
            <>
